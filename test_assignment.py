...
def import_notebook_module(notebook_path):
    ...
    with open(notebook_path, 'r') as f:
        notebook_content = nbconvert.PythonExporter().from_filename(notebook_path)[
            0]
...
def test_while_loop_even_numbers(capsys):
    ...
    assert len(captured) == len(
        expected), "Incorrect number of printed numbers"
...
def test_number_classification():
    ...
    # Test cases
    assert "negative" in test_input(
        -5).lower(), "Failed to identify negative number"
    assert "positive" in test_input(
        10).lower(), "Failed to identify positive number"
...
def test_multiplication_table(capsys):
    ...
    for actual, expect in zip(captured[:5], expected_start):
        assert actual.strip(
        ) == expect, f"Incorrect multiplication table start: {actual}"
